Imports System
Imports PDSA.Validation

''' <summary>
''' This class contains properties for each data value returned, or parameter to be input/output from the spStocks2 stored procedure.
''' This class is generated by the Haystack Code Generator for .NET.
''' Do not modify this class or add methods as it is intended to be able to be re-generated at any ti
''' </summary>

Partial Public Class spStocks2
    Inherits PDSAEntityBase

#Region "Constructor"
    Public Sub New()
        ClassName = "spStocks2"
    End Sub
#End Region

#Region "Private Variables"
    Private _bcode As String = String.Empty
    Private _sInsertid As String = String.Empty
    Private _RETURNVALUE As Integer = 0
#End Region

#Region "Public Properties"
    ''' <summary>
    ''' Get/Set the bcode value
    ''' </summary>
   _
    Public Property bcode() As String
        Get
            Return _bcode
        End Get
        Set(ByVal Value As String)
            If HasValueChanged(_bcode, Value, "@bcode") Then
                _bcode = Value
                RaisePropertyChanged("bcode")
            End If
        End Set
    End Property
    ''' <summary>
    ''' Get/Set the sInsertid value
    ''' </summary>
   _
    Public Property sInsertid() As String
        Get
            Return _sInsertid
        End Get
        Set(ByVal Value As String)
            If HasValueChanged(_sInsertid, Value, "@sInsertid") Then
                _sInsertid = Value
                RaisePropertyChanged("sInsertid")
            End If
        End Set
    End Property
    ''' <summary>
    ''' Get/Set the RETURNVALUE value
    ''' </summary>
   _
    Public Property RETURNVALUE() As Integer
        Get
            Return _RETURNVALUE
        End Get
        Set(ByVal Value As Integer)
            If HasValueChanged(_RETURNVALUE, Value, "@RETURN_VALUE") Then
                _RETURNVALUE = Value
                RaisePropertyChanged("RETURNVALUE")
            End If
        End Set
    End Property
#End Region
End Class
